...
;// -- Table: author
std::unique_ptr<t_btree_4__0_1_2_3__15> rel_1_author = std::make_unique<t_btree_4__0_1_2_3__15>();
souffle::RelationWrapper<0,t_btree_4__0_1_2_3__15,Tuple<RamDomain,4>,4> wrapper_rel_1_author;
// -- Table: inv_0
...
public:
Sf_program_small() : 
wrapper_rel_1_author(*rel_1_author,symTable,"author",std::array<const char *,4>{{"s:V","s:V","i:number","i:number"}},std::array<const char *,4>{{"v0","v1","@rule_number","@level_number"}}),
...
addRelation("author",&wrapper_rel_1_author,false,true);
...
IOSystem::getInstance().getWriter(std::vector<bool>({1,1,0,0}), symTable, ioDirectives, true)->writeAll(*rel_1_author);
...
std::cout << "Relation rel_1_author:\n";
rel_1_author->printHintStatistics(std::cout,"  ");
std::cout << "\n";
...
ioDirectives.setRelationName("rel_1_author");
IOSystem::getInstance().getWriter(std::vector<bool>({1,1,0,0}), symTable, ioDirectives, true)->writeAll(*rel_1_author);
} catch (std::exception& e) {std::cerr << e.what();exit(1);}
try {IODirectives ioDirectives;
ioDirectives.setIOType("stdout");
