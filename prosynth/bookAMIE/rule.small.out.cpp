...
;// -- Table: genre
std::unique_ptr<t_btree_4__0_1_2_3__15> rel_1_genre = std::make_unique<t_btree_4__0_1_2_3__15>();
souffle::RelationWrapper<0,t_btree_4__0_1_2_3__15,Tuple<RamDomain,4>,4> wrapper_rel_1_genre;
// -- Table: inv_0
...
public:
Sf_rule_small() : 
wrapper_rel_1_genre(*rel_1_genre,symTable,"genre",std::array<const char *,4>{{"s:V","s:V","i:number","i:number"}},std::array<const char *,4>{{"v0","v1","@rule_number","@level_number"}}),
...
addRelation("genre",&wrapper_rel_1_genre,false,true);
...
IOSystem::getInstance().getWriter(std::vector<bool>({1,1,0,0}), symTable, ioDirectives, true)->writeAll(*rel_1_genre);
...
std::cout << "Relation rel_1_genre:\n";
rel_1_genre->printHintStatistics(std::cout,"  ");
std::cout << "\n";
...
ioDirectives.setRelationName("rel_1_genre");
IOSystem::getInstance().getWriter(std::vector<bool>({1,1,0,0}), symTable, ioDirectives, true)->writeAll(*rel_1_genre);
} catch (std::exception& e) {std::cerr << e.what();exit(1);}
try {IODirectives ioDirectives;
ioDirectives.setIOType("stdout");
